...
# Виды связей SQLAlchemy

    OTM (1:n) - один ко многим
    MTO (n:1) - многие к одному
    MTM (n:n) - многие ко многим
    OTO (1:1) - один к одному
Каждое отношение имеет две стороны: прямую и обратную

Прямая сторона - кто является владельцем, инициатором

ForeignKey('table_name.pk_name') - использует СУБД для связи между таблицами

relationship() - использует SQLAlchemy для связи между сущностями	

1. Один к одному (1:1)

В этой связи каждой записи в одной таблице соответствует ровно одна запись в другой таблице. Например, если у вас есть таблица пользователей и таблица профилей, то каждый пользователь может иметь только один профиль, и каждый профиль принадлежит только одному пользователю. Пример:
...
В этой связи одной записи в первой таблице может соответствовать несколько записей во второй таблице. Например, один автор может написать несколько книг, но каждая книга принадлежит только одному автору. Пример:

    Таблица Authors:
        ID: 1, Имя: "Иван"
    Таблица Books:
        ID: 1, Title: "Книга 1", AuthorID: 1
        ID: 2, Title: "Книга 2", AuthorID: 1

3. Многие ко многим (N:M)

В этой связи одной записи в первой таблице может соответствовать несколько записей во второй таблице и наоборот. Например, студенты могут записываться на несколько курсов, а каждый курс может иметь нескольких студентов. Пример:

    Таблица Students:
        ID: 1, Имя: "Мария"
    Таблица Courses:
        ID: 1, Название: "Математика"
    Связующая таблица Enrollments:
